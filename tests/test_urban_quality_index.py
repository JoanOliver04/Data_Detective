...
@pytest.mark.parametrize("temp,esperado", [
    (20.0, 10.0), (18.0, 10.0), (25.0, 10.0), (0.0, 0.0), (45.0, 0.0),
])
...
@pytest.mark.parametrize("h,esperado", [
    (55.0, 10.0), (40.0, 10.0), (70.0, 10.0), (0.0, 0.0), (100.0, 0.0),
])
...
@pytest.mark.parametrize("mm,esperado", [
    (0.0, 10.0), (0.5, 10.0), (5.0, 5.0), (15.0, 2.0),
])
...
def test_score_trafico_niveles():
    ...
    assert _score_trafico(
        {"incidencias": [{"severidad": "high"}, {"severidad": "low"}]}
    )["score"] == 4.0
...
@pytest.mark.parametrize("score,nivel", [
    (9.0, "Excelente"), (7.5, "Bueno"), (6.0, "Aceptable"),
    (3.5, "Deficiente"), (2.0, "Malo"), (0.5, "Crítico"),
])
...
def test_indice_urbano_contam_rt_vacio():
    ...
    r = calcular_indice_urbano(contam_rt={"estaciones": []},
                               trafico_rt={"incidencias": []})
    assert r is not None
