...
def parse_gva_file(
    ...
    if lines is None:
        logger.error(
            f"No se pudo leer el archivo con ninguna codificación: {file_path.name}"
        )
    ...
    logger.info(
        f"Archivo parseado: {file_path.name} -> {len(df)} filas, {len(df.columns)} columnas"
    )
...
def transform_to_long_format(
    df: pd.DataFrame, station_code: str, logger: logging.Logger
) -> pd.DataFrame:
    """
    ...
    if not variables_to_process:
        logger.warning(
            f"No se encontraron variables de interés. Columnas disponibles: {variable_columns}"
        )
...
╚══════════════════════════════════════════════════════════════════════════════╝
    ...
    for station_code, dfs in data_by_station.items():
        ...
        nombre = station_names.get(
            station_code, ESTACIONES_VALENCIA.get(station_code, "Desconocida")
        )
