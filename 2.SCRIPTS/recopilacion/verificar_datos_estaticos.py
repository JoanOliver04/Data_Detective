...
"""
    ...
    for fuente, resultado in resultados.items():
        ...
        if resultado.get("fecha_min_global") and resultado.get("fecha_max_global"):
            periodo = (
                f"{resultado['fecha_min_global']} → {resultado['fecha_max_global']}"
            )
        else:
            periodo = "N/A"

        content += (
            f"| {nombre} | {tiene_datos} | {tiene_doc} | {registros} | {periodo} |\n"
        )
...
"""
    ...
    fuentes_documentadas = sum(
        1 for r in resultados.values() if r.get("tiene_documentacion")
    )
    ...
    for fuente in FUENTES_ESPERADAS.keys():
        ...
        if resultado["existe"]:
            ...
            if resultado["fecha_min_global"]:
                logger.info(
                    f"  Periodo: {resultado['fecha_min_global']} → {resultado['fecha_max_global']}"
                )

            if resultado["archivos_vacios"]:
                logger.warning(
                    f"  ⚠ Archivos vacíos: {len(resultado['archivos_vacios'])}"
                )
        else:
