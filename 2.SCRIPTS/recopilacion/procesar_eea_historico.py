...
def process_parquet_file(
    file_path: Path, logger: logging.Logger, city_only: bool = False
    ...
    try:
        ...
        df_valencia = df[
            df["Samplingpoint"].apply(lambda x: is_valencia_station(x, city_only))
        ]
        ...
        df_filtered = df_valencia[
            df_valencia["Pollutant"].apply(is_pollutant_of_interest)
        ]
...
╚══════════════════════════════════════════════════════════════════════════════╝
    ...
    if not all_data:
        ...
        logger.info(
            "Verifica que los archivos Parquet contienen estaciones de Valencia (código 462XXXXX)"
        )
        return
