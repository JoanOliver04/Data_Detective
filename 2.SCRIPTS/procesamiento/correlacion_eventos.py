...
EVENTOS_PATH = (
    PROJECT_ROOT / "1.DATOS_EN_CRUDO" / "eventos" / "eventos_clasificados.json"
)
...
def build_daily_aggregations(
    ...
    if df_contam is not None and not df_contam.empty:
        ...
        contam_diaria = df_ok.groupby(["fecha", "variable"], as_index=False).agg(
    ...
    if df_trafico is not None and not df_trafico.empty:
        ...
        trafico_diario = df_traf.groupby("fecha_dia", as_index=False).agg(
        ...
        logger.info(f"      âœ“ {len(trafico_diario):,} dÃ­as con datos de trÃ¡fico")
    else:
    ...
    if df_meteo is not None and not df_meteo.empty:
        ...
        meteo_diaria = df_met.groupby("fecha_dia", as_index=False).agg(
        ...
        logger.info(f"      âœ“ {len(meteo_diaria):,} dÃ­as con datos meteorolÃ³gicos")
    else:
...
def parse_and_deduplicate_events(
    ...
    for evento in eventos_raw:
        try:
            ...
            nombre = evento.get("nombre", evento.get("rival", "Evento desconocido"))
            ...
            parsed.append(
                {
                    ...
                    "categoria_evento": evento.get(
                        "categoria_evento", evento.get("tipo", "desconocido")
                    ),
                    "subcategoria_evento": evento.get(
                        "subcategoria_evento", "sin_clasificar"
                    ),
...
def _build_baseline_mask(
    ...
    if meteo_diaria is not None and not meteo_diaria.empty:
        ...
        meteo_lookup = meteo_diaria.set_index(meteo_diaria["fecha"].dt.date)[
            "precip_media"
        ]
...
def compute_event_impact(
    ...
    logger.info(f"  Fechas cubiertas por eventos: {len(all_event_dates)} dÃ­as Ãºnicos")
    ...
    for i, evento in enumerate(events):
        ...
        if trafico_diario is not None and not trafico_diario.empty:
            ...
            media_traf_evento = (
                traf_ev["n_incidencias"].mean() if not traf_ev.empty else np.nan
            )
            ...
            media_traf_baseline = (
                traf_bl["n_incidencias"].mean() if not traf_bl.empty else np.nan
            )
        ...
        if variables_contam:
            ...
            for variable in variables_contam:
                ...
                media_evento_val = (
                    datos_ev["valor_medio"].mean() if not datos_ev.empty else np.nan
                )
                n_dias_ev_real = (
                    datos_ev["fecha"].nunique() if not datos_ev.empty else 0
                )
                ...
                media_baseline_val = (
                    datos_bl["valor_medio"].mean() if not datos_bl.empty else np.nan
                )
                n_dias_bl = datos_bl["fecha"].nunique() if not datos_bl.empty else 0
                ...
                results.append(
                    {
                        ...
                        "categoria_evento": evento.get(
                            "categoria_evento", evento["tipo_evento"]
                        ),
                        "subcategoria_evento": evento.get(
                            "subcategoria_evento", "sin_clasificar"
                        ),
                        ...
                        "variable": variable,
                        "media_evento": (
                            round(media_evento_val, 2)
                            if not np.isnan(media_evento_val)
                            else np.nan
                        ),
                        "media_baseline": (
                            round(media_baseline_val, 2)
                            if not np.isnan(media_baseline_val)
                            else np.nan
                        ),
                        "impacto_pct": (
                            round(impacto_pct, 2)
                            if not np.isnan(impacto_pct)
                            else np.nan
                        ),
                        "n_dias_evento": n_dias_ev_real,
                        "n_dias_baseline": n_dias_bl,
                        "media_temp_evento": (
                            round(media_temp_evento, 1)
                            if not np.isnan(media_temp_evento)
                            else np.nan
                        ),
                        "media_temp_baseline": (
                            round(media_temp_baseline, 1)
                            if not np.isnan(media_temp_baseline)
                            else np.nan
                        ),
                        "media_precip_evento": (
                            round(media_precip_evento, 2)
                            if not np.isnan(media_precip_evento)
                            else np.nan
                        ),
                        "media_precip_baseline": (
                            round(media_precip_baseline, 2)
                            if not np.isnan(media_precip_baseline)
                            else np.nan
                        ),
                        "impacto_trafico_pct": (
                            round(impacto_trafico_pct, 2)
...
def save_results(
    ...
    try:
        ...
        logger.info(f"    TamaÃ±o:   {OUTPUT_FILE.stat().st_size / 1024:.1f} KB")
        return OUTPUT_FILE
    except Exception as e:
...
def print_summary(
    ...
    if n_traf > 0:
        ...
        traf_por_evento = df[trafico_valid].drop_duplicates(subset=["evento_id"])[
            "impacto_trafico_pct"
        ]
        logger.info("")
        logger.info(
    ...
    if not valid_impacto.empty:
        ...
        top = (
            valid_impacto.groupby(["evento_id", "nombre_evento"], as_index=False)
            .agg(impacto_medio=("impacto_pct", "mean"))
            .nlargest(5, "impacto_medio")
        )
        for _, row in top.iterrows():
            logger.info(
                f"    {row['nombre_evento'][:45]:>45}: " f"{row['impacto_medio']:+.1f}%"
            )
